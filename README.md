...
- **视觉模型**(可选):截图 OCR 使用,需多模态模型,如 `kimi-latest`、`qwen-vl-max`;留空则与对话模型共用
...
- **学习进度追踪**:章节按播放会话/笔记/截图自动标记"学过","总览"页展示连续学习天数、本周学习时长(目标分钟数可在设置调整)、"继续学习"一键回到上次章节和每门课的进度条。
- **全局笔记页**:控制台"笔记"页按课程分组展示全部笔记,支持按课程和学习标签(考点/高频/简答/疑问)筛选;字幕列表每行可一键"转笔记"。
- **章节状态徽标**:课程详情页章节树显示每章状态(已学习/有字幕/有笔记/已摘要/已出题),一眼看出每章的加工进度。
- **截图 OCR**:配置视觉模型后,可将章节截图批量识别为字幕段(无字幕章节也能用 AI 摘要/出题/问答),也可在截图查看器单张识别文字;识别结果缓存复用,重复执行不重复生成。
